...
import { ServerSentEventGenerator as AbstractSSEGenerator } from './abstract.ts'
import { type DatastarEventOptions, type EventType, sseHeaders } from './types.ts'
...
export class ServerSentEventGenerator extends AbstractSSEGenerator {
  ...
  static stream(
    ...
    const readableStream = new ReadableStream({
      ...
      async cancel(reason) {
        const abortResult = options?.onAbort ? options.onAbort(reason) : null
        if (abortResult instanceof Promise) await abortResult
      },
    })
