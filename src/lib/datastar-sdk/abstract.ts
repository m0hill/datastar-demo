...
import { DefaultExecuteScriptAttributes, DefaultSseRetryDurationMs } from './consts.ts'
import {
...
export abstract class ServerSentEventGenerator {
  ...
  public mergeSignals(
    ...
    const dataLines = this.eachOptionIsADataLine(eventOptions).concat(
      this.eachNewlineIsADataLine('signals', signals)
    )
  ...
  public removeSignals(
    ...
    const pathsArray =
      typeof paths === 'string' ? paths.split(' ') : paths.flatMap(path => path.split(' '))
